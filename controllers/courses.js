...
exports.updateCourse=asyncHandler(async (req,res,next)=>{
    ...
    if(course.user.toString()!==req.user.id && req.user.role!=='publisher'){
        return next(new ErrorResponse(`User ${req.params.id} is not authorized to update course ${course._id}`,401))
    }
...
exports.deleteCourse=asyncHandler(async (req,res,next)=>{
    ...
    //Make sure user is course owner
    if(course.user.toString()!==req.user.id && req.user.role!=='publisher'){
        return next(new ErrorResponse(`User ${req.params.id} is not authorized to delete course ${course._id}`,401))
    }
    course.remove();
